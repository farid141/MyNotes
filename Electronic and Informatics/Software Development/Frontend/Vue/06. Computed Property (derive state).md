...
Merupakan sebuah konsep yang `hampir mirip` dengan `derived-state` atau `useMemo` dalam react yang memanfaatkan update sebuah state. Akan membuat nilai berdasarkan perubahan state yang membentuk nilai tersebut. Computed `sangat teroptimasi` seperti `useMemo`, hanya akan dijalankan ketika dependensi statenya ada yang berubah.
