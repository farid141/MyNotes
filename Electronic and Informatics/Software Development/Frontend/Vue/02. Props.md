...
Adalah sebuah cara untuk mentransfer variable/fungsi dari komponen parent ke child. Untuk menggunakannya, tuliskan `defineProps` pada child component.

## Naming convention

Sebenarnya dapat sesuai dengan name variabel yang dikirim, tetapi konvensionalnya:

- dikirim dengan `kebab-case`
- diakses child-komponen dengan `camelCase`
...
- Untuk `child ke parent` biasanya menggunakan `emit`, walaupun bisa dengan cara seperti react yang pass `function props`.

## Strict typing

Vue mendukung strict typing untuk meminimalisir runtime error.
...
// Dengan default value, kita wrap dengan 
const {propA, propB, propE, propF, propH} = withDefaults(defineProps<Props>(), {
  propA: undefined,
  propB: undefined,
