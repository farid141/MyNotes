...
Digunakan untuk fetching dan caching data. Ini seperti membuat hooks untuk fetch data, dimana kita dapat mengakses state dan response dari fetch.
...
- `queryFn`: fungsi `async` yang mengembalikan data dan `raise error` (lebih simple pake Axios karena response selain 200 sudah diraise error)
